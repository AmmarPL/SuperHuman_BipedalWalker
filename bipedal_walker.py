#This file needs to modified in your pip installation. Its path in my laptop is /Users/ammarpl/miniconda3/lib/python3.10/site-packages/gym/envs/box2d/bipedal_walker.py
import sys
import math
...
class BipedalWalker(gym.Env, EzPickle):
    ...
    def step(self, action):
        ...
        motor= 0
        for a in action:
            reward -= 0.00035 * MOTORS_TORQUE * np.clip(np.abs(a), 0, 1) #Plus means we are making it move too much
            motor -= 0.00035 * MOTORS_TORQUE * np.clip(np.abs(a), 0, 1)
            # reward += 0
        ...
        # print("if we added motor torque:",reward + motor)
        return np.array(state, dtype=np.float32), reward, done, {"fall":fall,"motor":motor}
